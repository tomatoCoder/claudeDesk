...
<template>
  <article class="message" :class="role">
    ...
    <div v-if="role === 'user'" class="avatar" aria-hidden="true">你</div>
...
<style scoped>
.message { display: flex; width: 100%; padding: 11px 0; }.message.user { align-items: flex-start; justify-content: flex-end; gap: 9px; }.message.assistant { justify-content: flex-start; }.avatar { display: grid; width: 27px; height: 27px; flex: 0 0 27px; place-items: center; border-radius: 8px; background: var(--surface-raised); color: var(--text-secondary); font-size: 10px; font-weight: 700; }.body { min-width: 0; line-height: 1.7; }.user-bubble { max-width: min(74%, 620px); padding: 9px 13px; border: 1px solid rgba(255,255,255,.055); border-radius: 15px 15px 5px 15px; background: #27241f; color: var(--text-primary); box-shadow: 0 7px 22px rgba(0,0,0,.12); }.assistant-content { width: 100%; padding: 5px 36px 5px 0; color: var(--text-primary); }.plain { white-space: pre-wrap; overflow-wrap: anywhere; }
.markdown :deep(p) { margin: 0 0 13px; }.markdown :deep(p:last-child) { margin-bottom: 0; }.markdown :deep(pre) { overflow: auto; padding: 13px; border: 1px solid var(--border-subtle); border-radius: 9px; background: #090908; font-family: var(--font-mono); font-size: 12px; }.markdown :deep(code) { padding: 1px 4px; border-radius: 4px; background: rgba(255,255,255,.045); font-family: var(--font-mono); color: #f2c7a6; }.markdown :deep(pre code) { padding: 0; background: none; }.markdown :deep(a) { color: #e9a16c; }.markdown :deep(ul), .markdown :deep(ol) { padding-left: 22px; }
