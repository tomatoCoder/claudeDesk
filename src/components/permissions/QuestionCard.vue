...
<template>
  <section class="question-card">
    <div v-for="question in questions" :key="question.question" class="question">
      ...
      <input v-model="custom[question.question]" type="text" :placeholder="t('otherAnswer')" />
    </div>
    <footer><button class="danger-button" type="button" @click="emit('deny')">{{ t('cancel') }}</button><button class="primary-button" type="button" @click="submit">{{ t('submitAnswer') }}</button></footer>
  </section>
</template>

<style scoped>
.question-card { margin: 14px 40px; padding: 15px; border: 1px solid var(--border-strong); border-radius: var(--radius-md); background: var(--surface-raised); }.question + .question { margin-top: 18px; }.header { color: var(--accent); font-size: 10px; font-weight: 700; letter-spacing: .08em; text-transform: uppercase; }.question h3 { margin: 5px 0 10px; font-size: 14px; }.option { display: flex; width: 100%; flex-direction: column; gap: 3px; margin: 5px 0; padding: 9px 10px; border: 1px solid var(--border-subtle); border-radius: var(--radius-sm); background: var(--surface-option); cursor: pointer; text-align: left; }.option.selected { border-color: var(--accent); background: var(--accent-soft); }.option small { color: var(--text-secondary); }.question input { width: 100%; margin-top: 6px; padding: 9px 10px; border: 1px solid var(--border-subtle); border-radius: var(--radius-sm); background: var(--surface-input); color: var(--text-primary); }.question-card footer { display: flex; justify-content: flex-end; gap: 8px; margin-top: 14px; }
</style>
